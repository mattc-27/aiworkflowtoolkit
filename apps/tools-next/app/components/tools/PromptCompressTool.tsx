...
import { boolFromParam, buildTokenCostUrl, buildCompressorUrl } from "../../../lib/links";

export default function PromptCompressTool() {
  const router = useRouter();
  const search = useSearchParams();

  const initialInput = search.get("input") ? decodeURIComponent(search.get("input") as string) : "";
  ...
  const result = useMemo(() => compressPrompt(input, { aggressive }), [input, aggressive]);
  ...
  function copyShareLink() {
    const url = `${window.location.origin}${buildCompressorUrl({ input, aggressive })}`;
  ...
  return (
    <div className="card tool-card tool-section">
      ...
      <textarea className="textarea textarea-xl" value={input} onChange={(e) => setInput(e.target.value)} />

      <div className="kv tool-section">
        <label>
          <input type="checkbox" checked={aggressive} onChange={(e) => setAggressive(e.target.checked)} /> Aggressive mode
        </label>
        <label>
          <input
            type="checkbox"
      ...
      <div className="actions">
        <button className="btn btn-secondary" onClick={copyShareLink}>Copy share link</button>
        <button className="btn btn-primary" onClick={goToTokenCost}>Recalculate cost with this compressed prompt</button>
      </div>
      ...
      <div className="help">
        Saved: <strong>{result.savedChars}</strong> chars (~{result.savedPercent}%).
      </div>
    </div>
  );
}
